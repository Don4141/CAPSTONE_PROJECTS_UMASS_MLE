...
def fetch_uniprot_info(uniqGene_name):
    ...
    """
    url = f"https://rest.uniprot.org/uniprotkb/search?query=gene_exact:{UniqGene_name} AND reviewed:true&fields=accession,id&format=tsv" #Constructs a URL to query Uniprot database with given gene name and fetches Uniprot ID if available.
    ...
    if response.status_code == 200:
        ...
        if len(lines) > 2 and lines[1].strip():
            ...
            return None, f"No results found for {UniqGene_name}."
    else:
        return None, f"Failed to fetch data for {UniqGene_name}: Status code {response.status_code}, Response: {response.text}"
...
if __name__ == "__main__":
    ...
    output_file = "./uniprot_results.txt"  # File to output the UniProt IDs and entry names
